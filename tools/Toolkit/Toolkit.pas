...
begin
  ...
  case AvailablePlatforms[TargetPlatform] of
    tpWindows: BuildWindows32;
    tpAndroid: BuildAndroid;
  end;
end;
...
procedure TProject.BuildAndroid;
  ...
  var cf: TCodeFile;
  var CompilerPath, JavaSrcPath, JavaDomain: String;
  var i, j, n: Integer;
  var b: Boolean;
...
begin
  ...
  JavaSource.Append('');
  JavaSource.Append('import javax.microedition.khronos.egl.EGL10;');
  JavaSource.Append('import javax.microedition.khronos.egl.EGLConfig;');
  JavaSource.Append('import javax.microedition.khronos.egl.EGLContext;');
  JavaSource.Append('import javax.microedition.khronos.egl.EGLDisplay;');
  ...
  JavaSource.Append('  public void onCreate (Bundle savedInstanceState) {');
  ...
  JavaSource.Append('    GLView = new G2MPView(getApplication().getApplicationContext());');
  JavaSource.Append('    setContentView(GLView);');
  JavaSource.Append('    GLView.setKeepScreenOn(true);');
  JavaSource.Append('    setTitle("' + TargetName + '");');
  JavaSource.Append('  }');
  JavaSource.Append('  @Override');
  ...
  JavaSource.Append('      Init(false, 0, 0);');
  JavaSource.Append('    }');
  JavaSource.Append('    public G2MPView(Context context, boolean translucent, int depth, int stencil) {');
  JavaSource.Append('      super(context);');
  JavaSource.Append('      Init(translucent, depth, stencil);');
  JavaSource.Append('    }');
  JavaSource.Append('    private void Init(boolean translucent, int depth, int stencil) {');
  JavaSource.Append('      if (translucent) {');
  JavaSource.Append('        this.getHolder().setFormat(PixelFormat.TRANSLUCENT);');
  JavaSource.Append('      }');
  JavaSource.Append('      setEGLContextFactory(new ContextFactory());');
  JavaSource.Append('      setEGLConfigChooser(');
  JavaSource.Append('        translucent ?');
  JavaSource.Append('        new ConfigChooser(8, 8, 8, 8, depth, stencil) :');
  JavaSource.Append('        new ConfigChooser(5, 6, 5, 0, depth, stencil)');
  JavaSource.Append('      );');
  JavaSource.Append('      setRenderer(new Renderer());');
  JavaSource.Append('    }');
  JavaSource.Append('    @Override');
  ...
  JavaSource.Append('    }');
  JavaSource.Append('    private class ContextFactory implements GLSurfaceView.EGLContextFactory {');
  JavaSource.Append('      private int EGL_CONTEXT_CLIENT_VERSION = 0x3098;');
  JavaSource.Append('      public EGLContext createContext(EGL10 egl, EGLDisplay display, EGLConfig eglConfig) {');
  JavaSource.Append('        int[] attrib_list = {EGL_CONTEXT_CLIENT_VERSION, 1, EGL10.EGL_NONE };');
  JavaSource.Append('        EGLContext context = egl.eglCreateContext(display, eglConfig, EGL10.EGL_NO_CONTEXT, attrib_list);');
  JavaSource.Append('        return context;');
  JavaSource.Append('      }');
  JavaSource.Append('      public void destroyContext(EGL10 egl, EGLDisplay display, EGLContext context) {');
  JavaSource.Append('        egl.eglDestroyContext(display, context);');
  JavaSource.Append('      }');
  JavaSource.Append('    }');
  JavaSource.Append('    private class ConfigChooser implements GLSurfaceView.EGLConfigChooser {');
  JavaSource.Append('      protected int mRedSize;');
  JavaSource.Append('      protected int mGreenSize;');
  JavaSource.Append('      protected int mBlueSize;');
  JavaSource.Append('      protected int mAlphaSize;');
  JavaSource.Append('      protected int mDepthSize;');
  JavaSource.Append('      protected int mStencilSize;');
  JavaSource.Append('      private int[] mValue = new int[1];');
  JavaSource.Append('      public ConfigChooser(int r, int g, int b, int a, int depth, int stencil) {');
  JavaSource.Append('        mRedSize = r;');
  JavaSource.Append('        mGreenSize = g;');
  JavaSource.Append('        mBlueSize = b;');
  JavaSource.Append('        mAlphaSize = a;');
  JavaSource.Append('        mDepthSize = depth;');
  JavaSource.Append('        mStencilSize = stencil;');
  JavaSource.Append('      }');
  JavaSource.Append('      private int EGL_OPENGL_ES_BIT = 0x01;');
  JavaSource.Append('      //private int EGL_OPENGL_ES2_BIT = 4;');
  JavaSource.Append('      private int[] s_configAttribs2 = {');
  JavaSource.Append('        EGL10.EGL_RED_SIZE, 4,');
  JavaSource.Append('        EGL10.EGL_GREEN_SIZE, 4,');
  JavaSource.Append('        EGL10.EGL_BLUE_SIZE, 4,');
  JavaSource.Append('        EGL10.EGL_RENDERABLE_TYPE, EGL_OPENGL_ES_BIT,');
  JavaSource.Append('        EGL10.EGL_NONE');
  JavaSource.Append('      };');
  JavaSource.Append('      public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display) {');
  JavaSource.Append('        int[] num_config = new int[1];');
  JavaSource.Append('        egl.eglChooseConfig(display, s_configAttribs2, null, 0, num_config);');
  JavaSource.Append('        int numConfigs = num_config[0];');
  JavaSource.Append('        if (numConfigs <= 0) {');
  JavaSource.Append('          throw new IllegalArgumentException("No configs match configSpec");');
  JavaSource.Append('        }');
  JavaSource.Append('        EGLConfig[] configs = new EGLConfig[numConfigs];');
  JavaSource.Append('        egl.eglChooseConfig(display, s_configAttribs2, configs, numConfigs, num_config);');
  JavaSource.Append('        return chooseConfig(egl, display, configs);');
  JavaSource.Append('      }');
  JavaSource.Append('      public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {');
  JavaSource.Append('        for (EGLConfig config : configs) {');
  JavaSource.Append('          int d = findConfigAttrib(egl, display, config, EGL10.EGL_DEPTH_SIZE, 0);');
  JavaSource.Append('          int s = findConfigAttrib(egl, display, config, EGL10.EGL_STENCIL_SIZE, 0);');
  JavaSource.Append('          if (d < mDepthSize || s < mStencilSize) continue;');
  JavaSource.Append('          int r = findConfigAttrib(egl, display, config, EGL10.EGL_RED_SIZE, 0);');
  JavaSource.Append('          int g = findConfigAttrib(egl, display, config, EGL10.EGL_GREEN_SIZE, 0);');
  JavaSource.Append('          int b = findConfigAttrib(egl, display, config, EGL10.EGL_BLUE_SIZE, 0);');
  JavaSource.Append('          int a = findConfigAttrib(egl, display, config, EGL10.EGL_ALPHA_SIZE, 0);');
  JavaSource.Append('          if (r == mRedSize && g == mGreenSize && b == mBlueSize && a == mAlphaSize) return config;');
  JavaSource.Append('        }');
  JavaSource.Append('        return null;');
  JavaSource.Append('      }');
  JavaSource.Append('      private int findConfigAttrib(EGL10 egl, EGLDisplay display, EGLConfig config, int attribute, int defaultValue) {');
  JavaSource.Append('        if (egl.eglGetConfigAttrib(display, config, attribute, mValue)) {');
  JavaSource.Append('          return mValue[0];');
  JavaSource.Append('        }');
  JavaSource.Append('        return defaultValue;');
  JavaSource.Append('      }');
  JavaSource.Append('    }');
  JavaSource.Append('    private class Renderer implements GLSurfaceView.Renderer {');
