...
procedure TProject.BuildAndroid;
  ...
  var cf: TCodeFile;
  var CompilerPath: String;
  var pc, ProjStr, LibStr: AnsiString;
  var i, j, n: Integer;
  var b: Boolean;
  const READ_BYTES = 2048;
begin
  ...
  begin
    ...
    begin
      b := False;
      for n := j to G2Min(j + Length(ProjStr), Length(pc)) do
      if LowerCase(pc[n]) <> ProjStr[n - j + 1] then
      begin
        b := True;
        Break;
      end;
    ...
    if not b then
    begin
      Move(LibStr[1], pc[j], Length(LibStr));
    end;
  end;
  cf.SetCode(G2StrReplace(pc, '{#project_name#}', GetProjectName));
  cf.Save(_FilePath + 'build' + G2PathSep + _FileName);
  cf.Finalize;
  CommandLine := '';
  AddOption('-Tandroid');
  ...
  AddOption('-FL"' + App.Settings.PathANDK + '\platforms\android-8\arch-arm\usr\lib"');
  AddOption('-FL"' + g2.AppPath + '\fpc\3.0.0\lib\eabi"');
  //AddOption('-FL"' + App.Settings.PathANDK + '\toolchains\arm-linux-androideabi-4.6\prebuilt\windows\lib\gcc\arm-linux-androideabi\4.6"');
  //AddUnit(g2.AppPath + '\fpc\3.0.0\bin\i386-win32\rtl');
  //AddUnit(g2.AppPath + '\fpc\3.0.0\bin\i386-win32\winunits-base');
  //AddUnit(g2.AppPath + '\fpc\3.0.0\bin\i386-win32\paszlib');
  AddUnit(g2.AppPath + '\fpc\3.0.0\units\arm-android');
  AddUnit(g2.AppPath + '\fpc\3.0.0\units\arm-android\rtl');
  AddUnit(g2.AppPath + '\fpc\3.0.0\source\packages\paszlib\src');
  AddUnit(App.Settings.PathANDK + '\platforms\android-8\arch-arm\usr\lib');
  AddUnit(g2.AppPath + '..\..\source');//g2mp
  AddUnit(g2.AppPath + '..\..\source\box2d');
  AddUnit(g2.AppPath + '..\..\source\spine');
  AddOption('-FD' + g2.AppPath + '\fpc\3.0.0\bin\i386-win32');
  for i := 0 to High(_ProjectIncludeSource) do
  AddUnit(G2StrReplace(_ProjectIncludeSource[i], '$(project_root)', _FilePath));
  CommandLine := CommandLine + ' "' + _FilePath + 'build' + G2PathSep + _FileName + '"';
  CompilerProcess.sl := TStringList.Create;
  ...
  begin
    ...
    App.Log.Log('Build Failed');
    App.Console.AddLine('Build Failed');
    App.Console.Dump(g2.AppPath + 'g2console_dump.txt');
  end;
  CompilerProcess.Proc.Free;
  CompilerProcess.ProcOutput.Free;
  CompilerProcess.sl.Free;
