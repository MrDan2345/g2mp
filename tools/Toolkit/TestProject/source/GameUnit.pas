...
begin
  ...
  world.set_continuous_physics(true);
  ...
  draw := t_draw.create;
  draw.draw_flags := draw.draw_flags + [b2_df_shape, b2_df_joint, {b2_df_aabb, b2_df_pair,} b2_df_center_of_mass];
...
begin
  ...
  //c := world.get_contact_list;
  //while c <> nil do
  //begin
  //  c^.get_world_manifold(wm);
  //  disp.PrimCircleCol(
  //    wm.points[0].x, wm.points[0].y,
  //    0.1, $ffff0000, $ffff0000
  //  );
  //  c := c^.get_next;
  //end;
  //b := world.get_body_list;
  //while b <> nil do
  //begin
  //  vl := b^.get_linear_velocity;
  //  va := b^.get_angular_velocity;
  //  tp := disp.CoordToScreen(G2Vec2(b^.get_position.x, b^.get_position.y));
  //  //Font.Print(
  //  //  tp.x, tp.y,
  //  //  0.5, 0.5,
  //  //  '(' + FloatToStr(vl.x) + '; ' + FloatToStr(vl.y) + '); ' + FloatToStr(va)
  //  //);
  //  b := b^.get_next;
  //end;
