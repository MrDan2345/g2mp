...
type
  ...
  TSpineDataReader = class (TInterfacedObject)
  private
    _Buffer: PByteArray;
    _Size: Integer;
    _Pos: Integer;
  public
    constructor Create(const Buffer: Pointer; const BufferSize: Integer);
    destructor Destroy; override;
  end;

  TSpineFileProvider = class
  public
    class function FetchFile(const FileName: String): TSpineDataReader; virtual;
  end;
  CSpineFileProvider = class of TSpineFileProvider;
...
var
  SpineFileProvider: CSpineFileProvider = TSpineFileProvider;
...
constructor TSpineDataReader.Create(const Buffer: Pointer; const BufferSize: Integer);
begin
  inherited Create;
  _Buffer := PByteArray(Buffer);
  _Size := BufferSize;
  _Pos := 0;
end;

destructor TSpineDataReader.Destroy;
begin
  FreeMem(_Buffer, Size);
  inherited Destroy;
end;

class function TSpineFileProvider.FetchFile(const FileName: String): TSpineDataReader;
  var fs: TFileStream;
  var Buffer: Pointer;
  var Size: Integer;
begin
  ...
  try
    ...
    Buffer := GetMem(Size);
    fs.Read(Buffer, Size);
    Result := TSpineDataReader.Create(Buffer, Size);
  finally
    fs.Free;
  end;
end;
