...
type
  ...
  type tb2_draw_flags = (
    ...
    e_df_aabb_bit = $4,
    e_df_pair_bit = $8,
    e_df_center_of_mass_bit = $10
  );

  tb2_draw = class
  protected
    _draw_flags: tb2_uint32;
  public
    constructor create;
    destructor destroy; override;
    procedure set_flags(const flags: tb2_uint32); inline;
    function get_flags: tb2_uint32; inline;
    procedure append_flags(const flags: tb2_uint32); inline;
    procedure clear_flags(const flags: tb2_uint32); inline;
  ...
  private
    _data: array [0..b2_stack_size - 1] of tb2_int8;
    _index: tb2_int32;
    _allocation: tb2_int32;
    _max_allocation: tb2_int32;
    _entries: array [0..b2_max_stack_entries - 1] of tb2_stack_entry;
    _entry_count: tb2_int32;
  public
    constructor create;
    destructor destroy;
    function alloc(const size: tb2_int32): tb2_ptr;
  ...
  {$if defined(WIN32)}
    _start: tb2_float64;
    _inv_frequency: tb2_float64;
  {$elseif defined(UNIX)}
    _start_sec: tb2_uint32;
    _start_usec: tb2_uint32;
  {$endif}
  public
    constructor create;
    procedure reset;
  ...
  public
    ...
    destructor destroy; virtual;
    procedure clear;
    procedure create_loop(const new_vertices: pb2_vec2_arr; const vcount: tb2_int32);
    procedure create_chain(const new_vertices: pb2_vec2_arr; const vcount: tb2_int32);
  ...
  public
    ...
    var &type: tb2_separation_function_type;
    var local_point: tb2_vec2;
    var axis: tb2_vec2;
    function initialize(
      const cache: pb2_simplex_cache;
  ...
  tb2_body_flag = (
    e_bf_island_flag = $0001,
    e_bf_awake_flag = $0002,
    e_bf_auto_sleep_flag = $0004,
    e_bf_bullet_flag = $0008,
    e_bf_fixed_rotation_flag = $0010,
    e_bf_active_flag = $0020,
    e_bf_toi_flag = $0040
  );
  ...
  private
    _type: tb2_body_type;
    _flags: tb2_uint16;
    _island_index: tb2_int32;
    _xf: tb2_transform;
    _sweep: tb2_sweep;
    _linear_velocity: tb2_vec2;
    _angular_velocity: tb2_float32;
    _force: tb2_vec2;
    _torque: tb2_float32;
    _world: pb2_world;
    _prev: pb2_body;
    _next: pb2_body;
    _fixture_list: pb2_fixture;
    _fixture_count: tb2_int32;
    _joint_list: pb2_joint_edge;
    _contact_list: pb2_contact_edge;
    _mass, _rcp_mass: tb2_float32;
    _inertia, _rcp_inertia: tb2_float32;
    _linear_damping: tb2_float32;
    _angular_damping: tb2_float32;
    _gravity_scale: tb2_float32;
    _sleep_time: tb2_float32;
    _user_data: tb2_ptr;
  ...
  public
    ...
    procedure set_transform(const position: tb2_vec2; const angle: tb2_float32);
  ...
  tb2_contact_flags = (
    e_cf_island_flag = $0001,
    e_cf_touching_flag = $0002,
    e_cf_enabled_flag = $0004,
    e_cf_filter_flag = $0008,
    e_cf_bullet_hit_flag = $0010,
    e_cf_toi_flag = $0020
  );

  pb2_contact_listener = ^tb2_contact_listener;

  tb2_contact = object
  protected
    var _flags: tb2_uint32;
    ...
    procedure update(const listener: pb2_contact_listener);
  public
    constructor create;
  ...
  tb2_query_fixture_callback = function (const fixture: pb2_fixture): boolean;
  ...
  tb2_world_flags = (
    e_wf_new_fixture = $0001,
    e_wf_locked = $0002,
    e_wf_clear_forces = $0004
  );
...
var
  ...
  b2_contact_registers: array [0..longint(e_st_type_count) - 1, 0..longint(e_st_type_count) - 1] of tb2_contact_register;
...
procedure tb2_draw.set_flags(const flags: tb2_uint32);
begin
  _draw_flags := flags;
end;

function tb2_draw.get_flags: tb2_uint32;
begin
  result := _draw_flags;
end;

procedure tb2_draw.append_flags(const flags: tb2_uint32);
begin
  _draw_flags := _draw_flags or flags;
end;

procedure tb2_draw.clear_flags(const flags: tb2_uint32);
begin
  _draw_flags := _draw_flags and (not flags);
end;
//tb2_draw DND
...
begin
  case (shape^.get_type) of
    e_st_circle:
    begin
      vertices := pb2_vec2_arr(@circle^.cp);
      count := 1;
...
function tb2_circle_shape.test_point(const xf: tb2_transform; const p: tb2_vec2): boolean;
  var center, d: tb2_vec2;
begin
  center := xf.p + b2_mul(xf.q, cp);
  d := p - center;
...
begin
  ...
  mass_data.center := cp;
  mass_data.inertia := mass_data.mass * (0.5 * radius * radius + b2_dot(cp, cp));
end;
...
procedure tb2_chain_shape.create_loop(const new_vertices: pb2_vec2_arr; const vcount: tb2_int32);
begin
  count := vcount + 1;
...
procedure tb2_chain_shape.create_chain(const new_vertices: pb2_vec2_arr; const vcount: tb2_int32);
begin
  count := vcount;
  vertices := pb2_vec2_arr(b2_alloc(count * sizeof(tb2_vec2)));
  move(new_vertices^, vertices^, count * sizeof(tb2_vec2));
...
begin
  result := pb2_shape(allocator^.alloc(sizeof(tb2_chain_shape)));
  pb2_chain_shape(result)^.create;
  pb2_chain_shape(result)^.create_chain(vertices, count);
...
procedure tb2_broad_phase.update_pairs(const callback: tb2_add_pair_callback);
  procedure sort_pairs;
    var i, j, m: tb2_int32;
    var tmp: tb2_pair;
  begin
    for i := 0 to _pair_count - 2 do
    begin
      m := i;
      for j := i + 1 to _pair_count - 1 do
      if pair_less_than(_pair_buffer^[j], _pair_buffer^[m]) then m := j;
      if m <> i then
      begin
        tmp := _pair_buffer^[m];
        _pair_buffer^[m] := _pair_buffer^[i];
        _pair_buffer^[i] := tmp;
      end;
    end;
  end;
...
begin
  ...
  case &type of
    ...
    begin
      ...
      index_b^ := proxy_a^.get_support(axis_a);
...
begin
  _flags := 0;
  if def.bullet then
  _flags := _flags or longint(e_bf_bullet_flag);
  if def.fixed_rotation then
  _flags := _flags or longint(e_bf_fixed_rotation_flag);
  if def.allow_sleep then
  _flags := _flags or longint(e_bf_auto_sleep_flag);
  if def.awake then
  _flags := _flags or longint(e_bf_awake_flag);
  if def.active then
  _flags := _flags or longint(e_bf_active_flag);
  _world := world;
  _xf.p := def.position;
...
begin
  ...
  if _flags and longint(e_bf_active_flag) > 0 then
  begin
    broad_phase := @_world^._contact_manager.broad_phase;
  ...
  reset_mass_data;
  _world^._flags := _world^._flags or longint(e_wf_new_fixture);
end;
...
begin
  ...
  if wake and ((_flags and longint(e_bf_awake_flag)) = 0) then
  set_awake(true);
  if _flags and longint(e_bf_awake_flag) > 0 then
  begin
    _force := _force + force;
...
begin
  ...
  _mass := 1.0;
  _rcp_mass := 1.0 / _mass;
  if (data.inertia > 0.0) and ((_flags and longint(e_bf_fixed_rotation_flag)) = 0) then
  begin
...
begin
  ...
  if (_inertia > 0.0) and ((_flags and longint(e_bf_fixed_rotation_flag)) = 0) then
  begin
...
begin
  ...
  _flags := _flags and not longint(e_bf_bullet_flag);
end;

function tb2_body.is_bullet: boolean;
begin
  result := (_flags and longint(e_bf_bullet_flag)) = longint(e_bf_bullet_flag);
end;
...
begin
  ...
  begin
    _flags := _flags and not longint(e_bf_auto_sleep_flag);
    set_awake(true);
  end;
end;
...
begin
  result := (_flags and longint(e_bf_auto_sleep_flag)) = longint(e_bf_auto_sleep_flag);
end;
...
begin
  ...
  begin
    _flags := _flags and not longint(e_bf_awake_flag);
    _sleep_time := 0.0;
    _linear_velocity.set_zero;
    _angular_velocity := 0.0;
    _force.set_zero;
...
begin
  result := (_flags and longint(e_bf_awake_flag)) = longint(e_bf_awake_flag);
end;
...
begin
  ...
  begin
    _flags := _flags and not longint(e_bf_active_flag);
    broad_phase := @_world^._contact_manager.broad_phase;
    f := _fixture_list;
    while f <> nil do
    begin
...
begin
  result := (_flags and longint(e_bf_active_flag)) = longint(e_bf_active_flag);
end;

procedure tb2_body.set_fixed_rotation(const flag: boolean);
  var status: boolean;
begin
  status := (_flags and longint(e_bf_fixed_rotation_flag)) = longint(e_bf_fixed_rotation_flag);
  if status = flag then exit;
  if flag then
  _flags := _flags or longint(e_bf_fixed_rotation_flag)
  else
  _flags := _flags and not longint(e_bf_fixed_rotation_flag);
...
begin
  result := (_flags and longint(e_bf_fixed_rotation_flag)) = longint(e_bf_fixed_rotation_flag);
end;
...
begin
  _flags := _flags or longint(e_cf_filter_flag);
end;
...
begin
  add_type(@b2_create_circle_contact, @b2_destroy_circle_contact, e_st_circle, e_st_circle);
  add_type(@b2_create_polygon_and_circle_contact, @b2_destroy_polygon_and_circle_contact, e_st_polygon, e_st_circle);
  add_type(@b2_create_polygon_contact, @b2_destroy_polygon_contact, e_st_polygon, e_st_polygon);
  add_type(@b2_create_edge_and_circle_contact, @b2_destroy_edge_and_circle_contact, e_st_edge, e_st_circle);
  add_type(@b2_create_edge_and_polygon_contact, @b2_destroy_edge_and_polygon_contact, e_st_edge, e_st_polygon);
  add_type(@b2_create_chain_and_circle_contact, @b2_destroy_chain_and_circle_contact, e_st_chain, e_st_circle);
  add_type(@b2_create_chain_and_polygon_contact, @b2_destroy_chain_and_polygon_contact, e_st_chain, e_st_polygon);
end;
...
procedure tb2_contact.update(const listener: pb2_contact_listener);
...
begin
  ...
  _flags := _flags or longint(e_cf_enabled_flag);
  touching := false;
  was_touching := (_flags and longint(e_cf_touching_flag)) = longint(e_cf_touching_flag);
  ...
  if touching then
  _flags := _flags or longint(e_cf_touching_flag)
  else
  _flags := _flags and (not longint(e_cf_touching_flag));
...
begin
  _flags := longint(e_cf_enabled_flag);
  _fixture_a := f_a;
  _fixture_b := f_b;
  _index_a := index_a;
  _index_b := index_b;
...
begin
  result := (_flags and longint(e_cf_touching_flag)) = longint(e_cf_touching_flag);
end;
...
begin
  ...
  _flags := _flags and not longint(e_cf_enabled_flag);
end;

function tb2_contact.is_enabled: boolean;
begin
  result := (_flags and longint(e_cf_enabled_flag)) = longint(e_cf_enabled_flag);
end;
...
begin
  ...
  begin
    ...
    body_a := fixture_a^.get_body;
    body_b := fixture_b^.get_body;
    if (c^._flags and longint(e_cf_filter_flag)) > 0 then
    begin
	...
	continue;
      ...
      c^._flags := c^._flags and not longint(e_cf_filter_flag);
    end;
...
begin
  ...
  begin
    ...
    joints_okay := true;
    for j := 0 to _joint_count - 1 do
    joints_okay := joints_okay and _joints^[j]^.solve_position_constraints(solver_data);
  ...
  begin
    ...
    begin
      ...
      if (
         ((b^._flags and longint(e_bf_auto_sleep_flag)) = 0)
...
begin
  joint^.destroy;
  case joint^._type of
    e_jt_distance_joint: allocator^.free(joint, sizeof(tb2_distance_joint));
    e_jt_mouse_joint: allocator^.free(joint, sizeof(tb2_mouse_joint));
    e_jt_prismatic_joint: allocator^.free(joint, sizeof(tb2_prismatic_joint));
    e_jt_revolute_joint: allocator^.free(joint, sizeof(tb2_revolute_joint));
    e_jt_pulley_joint: allocator^.free(joint, sizeof(tb2_pulley_joint));
    e_jt_gear_joint: allocator^.free(joint, sizeof(tb2_gear_joint));
    e_jt_wheel_joint: allocator^.free(joint, sizeof(tb2_wheel_joint));
    e_jt_weld_joint: allocator^.free(joint, sizeof(tb2_weld_joint));
    e_jt_friction_joint: allocator^.free(joint, sizeof(tb2_friction_joint));
    e_jt_rope_joint: allocator^.free(joint, sizeof(tb2_rope_joint));
    e_jt_motor_joint: allocator^.free(joint, sizeof(tb2_motor_joint));
  end;
end;
...
begin
  _index_b := _body_b^._island_index;
  _local_anchor_b := _body_b^._sweep.local_center;
  _rcp_mass_b := _body_b^._rcp_mass;
  _rcp_inertia_b := _body_b^._rcp_inertia;
  c_b := data.positions^[_index_b].c;
  a_b := data.positions^[_index_b].a;
...
begin
  ...
  begin
    ...
    _limit_state := e_ls_equal_limits
    else if joint_angle <= _lower_angle then
    begin
      if _limit_state <> e_ls_at_lower_limit then
      _impulse.z := 0.0;
      _limit_state := e_ls_at_lower_limit;
    end
    else if joint_angle >= _upper_angle then
    begin
      if _limit_state <> e_ls_at_upper_limit then
      _impulse.z := 0.0;
      _limit_state := e_ls_at_upper_limit;
    end
    else
    begin
      _limit_state := e_ls_inactive_limit;
      _impulse.z := 0.0;
    end;
  end
  else
...
begin
  ...
  i_a := _rcp_inertia_a; i_b := _rcp_inertia_b;
  fixed_rotation := i_a + i_b = 0.0;
  if _enable_motor and (_limit_state <> e_ls_equal_limits) and not fixed_rotation then
  begin
    c_dot := w_b - w_a - _motor_speed;
    impulse := -_motor_mass * c_dot;
    old_impulse := _motor_impulse;
  ...
  if _enable_limit and (_limit_state <> e_ls_inactive_limit) and not fixed_rotation then
  begin
    ...
    if _limit_state = e_ls_equal_limits then
    _impulse := _impulse + impulse_1
    else if _limit_state = e_ls_at_lower_limit then
    begin
    ...
    else if _limit_state = e_ls_at_upper_limit then
    begin
      new_impulse := _impulse.z + impulse_1.z;
      if new_impulse > 0.0 then
      begin
...
begin
  ...
  m_a := _rcp_mass_a; m_b := _rcp_mass_b;
  i_a := _rcp_inertia_a; i_b := _rcp_inertia_b;
  if _enable_motor and (_limit_state <> e_ls_equal_limits) then
  begin
  ...
  c_dot_1.y := w_b - w_a;
  if _enable_limit and (_limit_state <> e_ls_inactive_limit) then
  begin
...
begin
  ...
  _mass := 0.0;
  if _type_a = e_jt_revolute_joint then
  begin
    _jv_ac.set_zero;
    _jw_a := 1.0;
    _jw_c := 1.0;
  ...
  if _type_b = e_jt_revolute_joint then
  begin
    _jv_bd.set_zero;
    _jw_b := _ratio;
    _jw_d := _ratio;
...
begin
  ...
  if c > 0.0 then
  _state := e_ls_at_upper_limit
  else
  _state := e_ls_inactive_limit;
...
begin
  ...
  while b <> nil do
  begin
    b^._flags := b^._flags and (not longint(e_bf_island_flag));
    b := b^._next;
  end;
  c := _contact_manager.contact_list;
  while c <> nil do
  begin
    c^._flags := c^._flags and (not longint(e_bf_island_flag));
    c := c^._next;
  end;
  j := _joint_list;
  while j <> nil do
  ...
  while seed <> nil do
  begin
    if seed^._flags and longint(e_bf_island_flag) > 0 then
    begin
      seed := seed^._next;
      continue;
    end;
    ...
    inc(stack_count);
    seed^._flags := seed^._flags or longint(e_bf_island_flag);
    ...
    begin
      ...
      begin
	c := ce^.contact;
	if c^._flags and longint(e_cf_island_flag) > 0 then
	begin
          ce := ce^.next;
	  continue;
	end;
	...
	island.add(c);
	c^._flags := c^._flags or longint(e_cf_island_flag);
	other := ce^.other;
	if other^._flags and longint(e_cf_island_flag) > 0 then
	begin
          ce := ce^.next;
	  continue;
	end;
	stack^[stack_count] := other;
        inc(stack_count);
	other^._flags := other^._flags or longint(e_bf_island_flag);
        ce := ce^.next;
      end;
      je := b^._joint_list;
      while je <> nil do
	...
	if other^._flags and longint(e_bf_island_flag) > 0 then
	begin
          je := je^.next;
	  continue;
	end;
  ...
  while b <> nil do
  begin
    if (b^._flags and longint(e_bf_island_flag)) = 0 then
    begin
      b := b^.get_next;
      continue;
    end;
...
begin
  ...
  begin
    ...
    while c <> nil do
    begin
      c^._flags := c^._flags and not (longint(e_cf_toi_flag) or longint(e_cf_island_flag));
      c^._toi_count := 0;
      c^._toi := 1.0;
      c := c^._next;
    end;
  ...
  begin
    ...
    begin
      ...
      alpha := 1.0;
      if c^._flags and longint(e_cf_toi_flag) > 0 then
      begin
	alpha := c^._toi;
      end
      else
	...
	b2_time_of_impact(output, input);
        {$hints on}
	beta := output.t;
	if output.state = e_tois_touching then
	alpha := b2_min(alpha0 + (1.0 - alpha0) * beta, 1.0)
	else
	alpha := 1.0;
	c^._toi := alpha;
	c^._flags := c^._flags or longint(e_cf_toi_flag);
      end;
    ...
    min_contact^._flags := min_contact^._flags and (not longint(e_cf_toi_flag));
    ...
    b_a^._flags := b_a^._flags or longint(e_bf_island_flag);
    b_b^._flags := b_b^._flags or longint(e_bf_island_flag);
    min_contact^._flags := min_contact^._flags or longint(e_cf_island_flag);
    ...
    begin
      ...
      while ce <> nil do
      begin
        ce^.contact^._flags := ce^.contact^._flags and (not (longint(e_cf_toi_flag) or longint(e_cf_island_flag)));
        ce := ce^.next
      end;
    end;
    _contact_manager.find_new_contacts;
...
begin
  ...
  case joint^.get_type of
    e_jt_distance_joint: _debug_draw.draw_segment(p1, p2, color);
    e_jt_pulley_joint:
...
begin
  ...
  _allow_sleep := true;
  _gravity := gravity;
  _flags := longint(e_wf_clear_forces);
  _rcp_dt0 := 0.0;
...
begin
  step_timer.create;
  if _flags and longint(e_wf_new_fixture) > 0 then
  begin
    _contact_manager.find_new_contacts;
    _flags := _flags and (not longint(e_wf_new_fixture));
  end;
  _flags := _flags or longint(e_wf_locked);
  s.dt := time_step;
  ...
  if (s.dt > 0.0) then
  _rcp_dt0 := s.rcp_dt;
  if _flags and longint(e_wf_clear_forces) > 0 then
  clear_forces;
  _flags := _flags and (not longint(e_wf_locked));
...
begin
  ...
  flags := _debug_draw.get_flags;
  if flags and longint(e_df_shape_bit) > 0 then
  begin
    b := _body_list;
    while b <> nil do
    begin
  ...
  if flags and longint(e_df_joint_bit) > 0 then
  begin
    j := _joint_list;
    while j <> nil do
    begin
  ...
  if flags and longint(e_df_pair_bit) > 0 then
  begin
  ...
  if flags and longint(e_df_aabb_bit) > 0 then
  begin
  ...
  if flags and longint(e_df_center_of_mass_bit) > 0 then
  begin
    b := _body_list;
    while b <> nil do
    begin
...
begin
  result := (_flags and longint(e_wf_locked)) = longint(e_wf_locked);
end;

procedure tb2_world.set_auto_clear_forces(const flag: boolean);
begin
  if flag then
  _flags := _flags or longint(e_wf_clear_forces)
  else
  _flags := _flags and not longint(e_wf_clear_forces);
end;

function tb2_world.get_auto_clear_forces: boolean;
begin
  result := (_flags and longint(e_wf_clear_forces)) = longint(e_wf_clear_forces);
end;
...
procedure tb2_world.shift_origin(const new_origin: tb2_vec2);
  ...
  var j: pb2_joint;
begin
  if _flags and longint(e_wf_locked) = longint(e_wf_locked) then exit;
  b := _body_list;
  while b <> nil do
  begin
...
begin
  ...
  p_a := b2_mul(xf_a, circle_a^.cp);
  p_b := b2_mul(xf_b, circle_b^.cp);
  d := p_b - p_a;
  dist_sqr := b2_dot(d, d);
  r_a := circle_a^.radius; r_b := circle_b^.radius;
  radius := r_a + r_b;
  if dist_sqr > radius * radius then exit;
  manifold.&type := e_mt_circles;
  manifold.local_point := circle_a^.cp;
  manifold.local_normal.set_zero;
  manifold.point_count := 1;
  manifold.points[0].local_point := circle_b^.cp;
...
procedure b2_collide_polygons(
  ...
  begin
    ...
    c^[0].id.cf.type_a := tb2_uint8(e_cft_face);
    c^[0].id.cf.type_b := tb2_uint8(e_cft_vertex);
    c^[1].v := b2_mul(xf2, vertices2^[i2]);
    c^[1].id.cf.index_a := tb2_uint8(edge1);
    c^[1].id.cf.index_b := tb2_uint8(i2);
    c^[1].id.cf.type_a := tb2_uint8(e_cft_face);
    c^[1].id.cf.type_b := tb2_uint8(e_cft_vertex);
  end;
...
begin
  manifold.point_count := 0;
  q := b2_mul_t(xf_a, b2_mul(xf_b, circle_b^.cp));
  ...
  cf.index_b := 0;
  cf.type_b := tb2_uint8(e_cft_vertex);
  if v <= 0.0 then
  begin
    p := a;
    d := q - p;
    ...
    cf.index_a := 0;
    cf.type_a := tb2_uint8(e_cft_vertex);
    manifold.point_count := 1;
    manifold.&type := e_mt_circles;
    manifold.local_normal.set_zero;
  ...
  n.normalize;
  cf.index_a := 0;
  cf.type_a := tb2_uint8(e_cft_face);
  manifold.point_count := 1;
  manifold.&type := e_mt_face_a;
...
procedure b2_collide_edge_and_polygon(
  ...
  begin
    ...
    if edge_axis.&type = e_at_unknown then exit;
    if edge_axis.separation > radius then exit;
    polygon_axis := compute_polygon_separation;
    if (polygon_axis.&type <> e_at_unknown) and (polygon_axis.separation > radius) then exit;
    if polygon_axis.&type = e_at_unknown then
    ...
    primary_axis := edge_axis;
    if primary_axis.&type = e_at_edge_a then
    begin
      manifold.&type := e_mt_face_a;
      best_index := 0;
	...
	begin
      ...
      ie[0].id.cf.type_a := tb2_uint8(e_cft_face);
      ie[0].id.cf.type_b := tb2_uint8(e_cft_vertex);
      ie[1].v := poly_b.vertices[i2];
      ie[1].id.cf.index_a := 0;
      ie[1].id.cf.index_b := tb2_uint8(i2);
      ie[1].id.cf.type_a := tb2_uint8(e_cft_face);
      ie[1].id.cf.type_b := tb2_uint8(e_cft_vertex);
      if front then
      begin
	rf.i1 := 0;
	rf.i2 := 1;
    ...
    begin
      ...
      ie[0].id.cf.type_a := tb2_uint8(e_cft_vertex);
      ie[0].id.cf.type_b := tb2_uint8(e_cft_face);
      ie[1].v := v2;
      ie[1].id.cf.index_a := 0;
      ie[1].id.cf.index_b := tb2_uint8(primary_axis.index);
      ie[1].id.cf.type_a := tb2_uint8(e_cft_vertex);
      ie[1].id.cf.type_b := tb2_uint8(e_cft_face);
      ...
      rf.normal := polygon_b^.normals[rf.i1];
    end;
...
begin
  ...
  begin
    ...
    v_out^[num_out].id.cf.type_a := byte(e_cft_vertex);
    v_out^[num_out].id.cf.type_b := byte(e_cft_face);
    inc(num_out);
  end;
  result := num_out;
end;
...
begin
  ...
  begin
    ...
    begin
      output.state := e_tois_overlapped;
      output.t := 0.0;
      break;
    end;
    if (distance_output.distance < target + tolerance) then
    begin
      output.state := e_tois_touching;
      output.t := t1;
      break;
    end;
